...
import com.seferdefteri.app.data.iceAktar
...
sealed interface GeriYukleSonuc {
    data class Tamam(val kayit: Int) : GeriYukleSonuc
    data object YedekYok : GeriYukleSonuc
    data class Hata(val mesaj: String) : GeriYukleSonuc
}

/**
 * Sunucudaki yedegi indirip telefona yazar.
 *
 * DIKKAT: telefondaki mevcut kayitlarin yerine gecer, birlestirmez.
 * Cagirmadan once kullaniciya onaylatilir.
 */
suspend fun geriYukle(context: android.content.Context): GeriYukleSonuc =
    kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) {
        val prefs = com.seferdefteri.app.Prefs(context)
        val anahtar = prefs.cihazAnahtari.trim()
        if (anahtar.isBlank()) return@withContext GeriYukleSonuc.Hata("Giris yapilmamis")

        var conn: java.net.HttpURLConnection? = null
        try {
            val adres = prefs.sunucuAdresi.trimEnd('/') + "/api/geri-yukle"
            conn = (java.net.URL(adres).openConnection() as java.net.HttpURLConnection).apply {
                requestMethod = "GET"
            ...
            val kod = conn.responseCode
            if (kod == 404) return@withContext GeriYukleSonuc.YedekYok
            if (kod !in 200..299) return@withContext GeriYukleSonuc.Hata("Sunucu hatasi ($kod)")

            val akis = if ((conn.contentEncoding ?: "").contains("gzip", true)) {
                java.util.zip.GZIPInputStream(conn.inputStream)
            } else {
                conn.inputStream
            }
            val metin = akis.bufferedReader().use { it.readText() }
            val veri = org.json.JSONObject(metin)
            val sayi = com.seferdefteri.app.data.Repo(context).iceAktar(veri)
            GeriYukleSonuc.Tamam(sayi)
        } catch (e: Exception) {
            GeriYukleSonuc.Hata(e.message ?: "Baglanti kurulamadi")
        } finally {
            conn?.disconnect()
        }
    }
